...
plot(datafull)
```
1.2. Iš karto pastebima, kad 2 rodikliai - garsoIzoliacija ir silumosLaidumas - labai susiję, todėl tikrinama jų koreliacija:
```{r}
cor(datafull)
```
...
3.3. Heteroskedastiškumo tikrinimas. Išbrėžiamos liekanos ir tikrinama pagal Breuch-Pagan testą:
```{r}
plot(mod2$res~mod2$fitted, main="Liekanų išsibarstymas pagal reikšmes", col=2)
ncvTest(mod2)
```
...
shapiro.test(mod2$res) #testas, kuris nustato paklaidų normalumą
```
Pagal shapiro.test p-value>0.05 priimame H0: paklaidos yra normalios

3.5.Tikinama liekatų autokoreliacija:
```{r}
